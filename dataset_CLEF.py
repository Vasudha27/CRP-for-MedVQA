"""
This code is modified based on Jin-Hwa Kim's repository (Bilinear Attention Networks - https://github.com/jnhwkim/ban-vqa) by Vasudha Joshi
"""
...
def _load_dataset(dataroot, name, img_id2val, label2ans):
    ...
    name: 'train', 'val', 'test'
    """
    data_path = os.path.join(dataroot, 'All_%s.csv'%name)
    if name == 'train':
        samples = pd.read_csv(data_path, sep='|', names=['image_name', 'qid', 'questions', 'answers','question_type','answer_type'])
    if name == 'val':
        samples = pd.read_csv(data_path, sep='|', names=['image_name', 'qid', 'questions', 'answers','question_type','answer_type'])
    if name == 'test':
        samples = pd.read_csv(data_path, sep='|', names=['image_name', 'answer_type', 'qid', 'question_type', 'questions', 'answers'])

    samples.sort_values(by = 'qid', inplace = True)

    answer_path = os.path.join(dataroot, 'new_cache', '%s_target.pkl' % name)
    answers = cPickle.load(open(answer_path, 'rb'))
    answers = sorted(answers, key=lambda x: x['qid'])

    utils.assert_eq(len(samples), len(answers))
    entries = []
    for sample, answer in zip(samples.iterrows(), answers):
        utils.assert_eq(sample[1]['qid'], answer['qid'])
        utils.assert_eq(sample[1]['image_name'], answer['image_name'])
        image_name = sample[1]['image_name']+".jpg"
        #print(image_name)
        if not COUNTING_ONLY or is_howmany(sample[1]['questions'], answer, label2ans):
            entries.append(_create_entry(img_id2val[image_name], sample[1], answer))
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, args, dictionary, dataroot='data', question_len=12):
        ...
        if args.use_CLEF:
            dataroot = args.CLEF_dir

        ans2label_path = os.path.join(dataroot, 'new_cache', 'trainval_ans2label.pkl')
        label2ans_path = os.path.join(dataroot, 'new_cache', 'trainval_label2ans.pkl')
        ...
        self.img_id2idx = json.load(open(os.path.join(dataroot, '%s_imgid2idx.json'% name)))

        self.entries = _load_dataset(dataroot, name, self.img_id2idx, self.label2ans)
        # load image data for MAML module
        if args.maml:
            # TODO: load images
            images_path = os.path.join(dataroot, '%s_images84x84.pkl'%name)
        ...
        if args.autoencoder:
            # TODO: load images
            images_path = os.path.join(dataroot, '%s_images128x128.pkl'%name)
            print('loading DAE image data from file: '+ images_path)
            self.ae_images_data = cPickle.load(open(images_path, 'rb'))

        if args.dir:
            images_path = os.path.join(dataroot, '%s_images224x224.pkl'%name)
            print('loading images of data from file: ' + images_path)
            self.dir_images_data = cPickle.load(open(images_path, 'rb'))
    ...
    def tensorize(self):
        ...
        if self.args.dir:
            self.dir_images_data = torch.from_numpy(self.dir_images_data)
            self.dir_images_data = self.dir_images_data.type('torch.FloatTensor')
    ...
    def __getitem__(self, index):
        ...
        if self.args.dir:
            mt_images_data = self.dir_images_data[entry['image']].reshape(3 * 224 * 224)
            image_data[2] = mt_images_data

        if None!=answer:
            labels = answer['labels'].to(torch.int64)
            scores = answer['scores']
            target = torch.zeros(self.num_ans_candidates)
            if labels is not None:
                target.scatter_(0, labels, scores)
            return image_name, image_data, qid, question, target, answer_type, question_type, None

        else:
            return image_data, question, answer_type, question_type, None
