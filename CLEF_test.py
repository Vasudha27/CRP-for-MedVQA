...
def parse_args():
    ...
    parser.add_argument('--weight', type=str, default=r'.\reported_models\pretrained_weight\roco_128_200_i500_r50_dir_best_checkpoint.pth', help='path to pretrained weights')
...
def get_result(model, dataloader, device, args):
    ...
    with torch.no_grad():
        ...
        for img_name, v, qid, q, a, ans_type, q_types, p_type in iter(dataloader):
...
if __name__ == '__main__':
    ...
    def process(args, model, eval_loader):
        model_path = args.input + '/model_epoch_%s.pth' % args.epoch
        ...
        if args.use_CLEF:
            result, quesntion_types_result, pred_label, actual_label  = get_result(model, eval_loader, args.device, args)
            print(args.input)
            ##################
            outfile_path = os.path.join(args.output , args.input.split(os.sep)[-1])
            outfile = os.path.join(outfile_path , 'results_%s.json' % args.epoch)
            ...
            precision = precision_score(actual_label, pred_label, average='weighted')
            recall = recall_score(actual_label, pred_label, average='weighted')
            f1 = f1_score(actual_label, pred_label, average='weighted')
            # Print results
